        ...
        <div className="mt-4 grid gap-2">
          {Array.from({ length: SAVE_SLOT_COUNT }, (_, slot) => {
            ...
            return (
              <div
                key={slot}
                className={`rounded-xl border p-3 ${active === slot ? 'border-neon-cyan/35 bg-neon-cyan/[0.06]' : 'border-white/[0.08] bg-white/[0.025]'}`}
              >
                <div className="flex flex-wrap items-center gap-3">
                  <div className="grid h-10 w-10 shrink-0 place-items-center rounded-lg border border-white/10 bg-black/20 font-mono text-sm text-white/60">
                    {slot + 1}
                  </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <input
              ...
              onChange={async (e) => {
                ...
                if (
                  !finalState.started ||
                  destination === finalState.activeSaveSlot ||
                  !sameSnapshot(existing, finalMeta)
                ) {
                  refresh();
                  setStatus({
                    tone: 'bad',
