...
export default function WholesalePanel({ vm }: { vm: CompanyModel }) {
  ...
  const rank = rankOf(vm.game);
  const next = nextRank(vm.game);
  const share = Math.round(cityShare(vm.game) * 100);
  return (
    <div className="panel panel-tone-violet p-5 lg:col-span-3">
      <div className="mb-3 flex items-baseline justify-between">
        <div>
          ...
          <p className="text-[11px] text-white/40">{tr ? rank.blurbTr : rank.blurb}</p>
        </div>
        <div className="text-right">
          <div className="text-lg font-semibold text-neon-cyan">{tr ? rank.nameTr : rank.name}</div>
          <div className="num text-[11px] text-white/40">{tr ? `Şehir payı %${share}` : `${share}% of the city`}</div>
        </div>
      </div>
      ...
      {next ? (
        <div className="mt-4">
          <div className="text-[11px] text-white/45">
            {tr ? 'Sıradaki:' : 'Next:'}{' '}
            <span className="font-semibold text-white/80">{tr ? next.nameTr : next.name}</span>
          </div>
              ...
              return (
                <div key={req.label} className="rounded-lg border border-white/10 bg-white/[0.03] p-2.5">
                  <div className="flex justify-between text-[11px]">
                    <span className="text-white/60">{tr ? req.labelTr : req.label}</span>
                    <span className={`num ${p >= 1 ? 'text-neon-lime' : 'text-white/45'}`}>
                      {req.detail(vm.game, tr)}
                    </span>
                  </div>
