  ...
  return (
    <div className={`panel panel-tone-amber p-5 lg:col-span-2 ${m.networkView === 'capacity' ? '' : 'hidden'}`}>
      ...
      <p className="mb-3 text-[11px] text-white/40">
        {tr
          ? 'Son zirvelerden geçen düz bir çizgi. Kapasite kurmak zaman alır; harekete geçmenin doğru anı çizgiler kesişmeden öncesidir.'
          : 'Straight line through recent peaks. Capacity takes time to build, so the useful moment to act is before the line crosses.'}
      </p>
      ...
      {m.demandSeries.length > 1 && (
        <div className="mb-4 rounded-lg border border-white/[0.07] bg-black/15 p-3">
          <TrendChart
            ...
            series={[
              { label: tr ? 'Günlük zirve Gbps' : 'Daily peak Gbps', values: m.demandSeries, color: '#2dd4bf' },
              {
                label: t(m.locale, 'accessCapacity'),
      ...
      {!m.forecast.confident ? (
        ...
        <>
                ...
                {m.daysLeft === null
                  ...
                  : m.daysLeft > 365
                    ? tr
                      ? '1 yıl+'
                      : '1y+'
                    : `${Math.round(m.daysLeft)}${tr ? ' gün' : 'd'}`}
              </div>
            </div>
          </div>
          ...
          <Meter
            ...
            label={tr ? 'Tahmini zirve ve kurulu kapasite' : 'Projected peak against what you have built'}
          ...
          {m.daysLeft !== null && m.daysLeft < 30 && (
            <div className="mt-3 rounded-lg border border-neon-red/40 bg-neon-red/10 p-3 text-[12px] text-neon-red">
              {tr
                ? `Mevcut eğilimle erişim kapasiten yaklaşık ${Math.round(m.daysLeft)} gün içinde dolacak.`
                : `On the current trend you run out of access capacity in about ${Math.round(m.daysLeft)} ${plural(Math.round(m.daysLeft), 'day')}.`}
            </div>
          )}
        </>
      )}
