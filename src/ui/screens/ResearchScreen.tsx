...
const BRANCH = {
  ...
  ops: { label: 'Operations', code: 'OP', color: '#f3b843', description: 'Automation, resilience and enterprise service.' },
} as const;
      ...
      <div className="mx-auto max-w-[1240px]">
            ...
            <div className="flex items-center justify-between gap-4 px-4 py-3">
              <div className="flex items-center gap-3">
                <div className="relative grid h-10 w-10 place-items-center rounded-full border border-neon-cyan/40 bg-neon-cyan/10 font-display text-[11px] font-semibold uppercase text-neon-cyan">
                  Live<span className="absolute inset-0 animate-ping rounded-full border border-neon-cyan/25" />
                </div>
                <div><div className="stat-label">In progress</div><div className="text-base font-semibold">{activeNode.name}</div></div>
              </div>
              <div className="text-right"><div className="num text-sm text-white/75">{Math.ceil(active!.daysLeft)} days</div><div className="text-[11px] text-white/35">remaining</div></div>
            </div>
            <div className="h-1 bg-white/5"><motion.div className="h-full bg-neon-cyan" animate={{ width: `${(1 - active!.daysLeft / activeNode.days) * 100}%` }} /></div>
          </div>
        )}
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          {(Object.keys(BRANCH) as Array<keyof typeof BRANCH>).map((branch) => {
            ...
            const branchReady = nodes.filter((r) => isAvailable(r, game.researchDone) && !game.researchDone.includes(r.id)).length;
            return (
              <section key={branch} className="relative rounded-xl border border-white/[0.07] bg-black/10 p-3">
                <header className="mb-3 flex items-start gap-3 rounded-lg bg-white/[0.025] p-3">
                  <div className="grid h-10 w-10 shrink-0 place-items-center rounded-lg border font-display text-sm font-semibold" style={{ color: meta.color, borderColor: `${meta.color}55`, background: `${meta.color}12` }}>{meta.code}</div>
                  <div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><h2 className="font-display text-lg font-semibold uppercase tracking-[0.1em]" style={{ color: meta.color }}>{meta.label}</h2><span className="font-mono text-[9px] text-white/35">{branchDone}/{nodes.length} ONLINE</span></div><p className="text-[11px] leading-snug text-white/40">{meta.description}</p>{branchReady > 0 && <div className="mt-1 text-[9px] font-semibold uppercase tracking-wider" style={{ color: meta.color }}>{branchReady} programme ready</div>}</div>
                </header>
                ...
                <div className="relative flex flex-col">
                  {nodes.map((r, index) => {
                    ...
                    const statusClass = done ? 'text-neon-lime' : activeHere ? 'text-neon-cyan' : available ? 'text-white/70' : 'text-white/[0.28]';
                    return (
                      <div key={r.id} className="relative pb-3 last:pb-0">
                        {index > 0 && <div className="absolute -top-3 left-6 h-3 w-px" style={{ background: `${meta.color}${done || available ? '99' : '33'}` }} />}
                        <div className={`relative overflow-hidden rounded-lg border p-3.5 transition-colors ${done ? 'bg-neon-lime/[0.025]' : activeHere ? 'bg-neon-cyan/[0.07]' : available ? 'bg-white/[0.035] hover:bg-white/[0.055]' : 'bg-black/20'}`} style={{ borderColor: done ? '#75df9a55' : activeHere ? '#2dd4bf77' : available ? `${meta.color}66` : 'rgba(255,255,255,.055)' }}>
                          <div className="absolute inset-y-0 left-0 w-0.5" style={{ background: done ? '#75df9a' : activeHere ? '#2dd4bf' : available ? meta.color : 'rgba(255,255,255,.08)' }} />
                          <div className="flex items-start gap-3">
                            <div className="relative mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-full border font-mono text-[10px]" style={{ borderColor: done ? '#75df9a88' : available ? `${meta.color}88` : 'rgba(255,255,255,.14)', color: done ? '#75df9a' : available ? meta.color : 'rgba(255,255,255,.3)' }}>
                              {done ? '✓' : r.tier}
                              {(done || available) && <span className="absolute -bottom-4 left-1/2 h-3 w-px -translate-x-1/2" style={{ background: index === nodes.length - 1 ? 'transparent' : `${meta.color}55` }} />}
                            </div>
                          ...
                          <div className="mt-2 flex flex-wrap gap-1.5">{r.unlocks.map((u) => <span key={u} className="rounded border border-white/[0.07] bg-white/[0.025] px-1.5 py-0.5 text-[10px] text-white/40">{u}</span>)}</div>
                          {!done && !activeHere && (
                            available ? (
                              ...
                              <div className="mt-3 rounded-md border border-white/[0.055] bg-black/20 px-2.5 py-2 text-[10px] leading-snug text-white/[0.34]">
                                <span className="mr-1 font-semibold uppercase tracking-wider text-white/25">Locked</span> Requires {requirements}
                              </div>
                            )
                          )}
                          {activeHere && <div className="mt-3 rounded-md border border-neon-cyan/20 bg-neon-cyan/[0.06] px-2.5 py-2 text-center text-[11px] font-semibold text-neon-cyan">Programme running</div>}
                        </div>
                      </div>
                    );
                  })}
