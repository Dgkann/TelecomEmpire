...
export default function SidePanel() {
  ...
  const priorityCount = insights.length + active.length + obligations.length + game.offers.length + (game.activeEvent ? 1 : 0);
    ...
    <>
      ...
      >
        <div className="pointer-events-auto panel flex items-center justify-between p-2.5 lg:hidden">
          <div><div className="text-[10px] uppercase tracking-widest text-neon-cyan">Action center</div><div className="text-[10px] text-white/35">Offers, alerts, events, and obligations</div></div>
          <button className="btn px-2 py-1 text-xs" onClick={() => setMobileOpen(false)} aria-label="Close action center">✕</button>
        </div>
              ...
              return (
                <button
                  ...
                  className={`relative rounded-md border px-1 py-1.5 text-center transition-colors ${
                    selected ? 'border-white/20 bg-white/10' : 'border-transparent bg-white/[0.025] hover:bg-white/[0.07]'
                  }`}
                >
                  <span className="num block text-[11px] font-semibold" style={{ color: section.tone }}>{section.count}</span>
                  <span className="block text-[8px] font-semibold uppercase tracking-wide text-white/45">{section.label}</span>
                  {selected && <span className="absolute inset-x-2 bottom-0 h-px" style={{ background: section.tone }} />}
                </button>
              );
            })}
          </div>
      ...
      {!game.activeEvent && insights.length === 0 && (
        <div className="panel p-4 text-center">
          ...
          <div className="mt-1 text-[10px] leading-snug text-white/40">No immediate operational priorities.</div>
        </div>
      )}
        </>
      )}
          ...
          return (
            ...
            >
              ...
              <div className="mt-1.5 text-[10px] leading-snug text-white/35">
                Flexible doubles the outage allowance for 15% less income. Premium asks 20% more, halves the bonus,
                and can lose the offer.
              </div>
            </motion.div>
          );
        })}
