...
test('small data centre connects, saves and expands through the research guidance', async ({ page }, testInfo) => {
  ...
  const site = await page.evaluate(() => {
    ...
    const cell = district.cells
      .filter((c) => !game.nodes.some((n) => n.gx === c.gx && n.gy === c.gy))
      .sort(
        (a, b) =>
  ...
  const id = await page.evaluate(({ gx, gy }) => {
    ...
    if (!store.getState().game.links.some((l) => l.aId === node.id || l.bId === node.id))
      throw new Error('No fibre connection');
    store.getState().upgradeNode(node.id);
    if (store.getState().game.nodes.find((n) => n.id === node.id)!.tier !== 0)
  ...
  expect(await page.evaluate(() => window.__game.getState().game.money)).toBe(3200000);
