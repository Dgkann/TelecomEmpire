...
test('fault finding saves the repair, pauses time and shares rewards with routing', async ({
  ...
  const fault = await page.evaluate(() =>
    window.__game.getState().game.signalTraining.active.rotations.findIndex((r: number) => r !== 0),
  );
  ...
  await page.reload();
  await page.evaluate(() => window.__game.getState().continueGame());
  ...
  expect(await page.evaluate(() => window.__game.getState().game.researchActive.daysLeft)).toBe(4);
  expect(
    await page.evaluate(() => {
      const store = window.__game;
  ...
  await page.evaluate(() => window.__game.getState().setScreen('projects'));
  ...
  const rotations: number[] = await page.evaluate(() => window.__game.getState().game.signalTraining.active.rotations);
  ...
  expect(await page.evaluate(() => window.__game.getState().game.researchActive.daysLeft)).toBe(4);
  expect(await page.evaluate(() => window.__game.getState().game.money)).toBe(before.money + 30000);
});
