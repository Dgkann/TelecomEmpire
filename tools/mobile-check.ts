  ...
  for (let i = 0; i < days * (MINUTES_PER_DAY / 5); i++) {
    ...
    for (const inc of g.incidents) {
      ...
      g.incidents = g.incidents.map((x) =>
        x.id === inc.id
          ? { ...x, repairMinutesLeft: Math.round(x.repairTotalMinutes * 0.28), assignedTechId: t.id }
          : x,
      );
...
console.log(
  '\ncoverage targets:',
  g.districts.map((d) => `${d.name} ${(mobileCoverageTarget(g, d) * 100).toFixed(0)}%`).join(' | '),
);
console.log(
  'tower traffic:',
  g.nodes
    .filter((n) => n.kind === 'tower')
    .map((n) => `${n.trafficGbps.toFixed(2)}/${n.capacityGbps.toFixed(1)}`)
    .join(' | '),
);
